...
def _ticket_db_id():
    db = os.getenv("NOTION_TICKET_DATABASE_ID",
                   os.getenv("NOTION_DATABASE_ID", ""))
    return db.replace("-", "").strip()


def create_notion_ticket(question: str, priority: str, department: str,
                         owner: str, thread_id: str, evidence_score: float,
                         sources_tried: list, conversation_summary: str) -> tuple:
    db_id = _ticket_db_id()
    if not db_id:
        raise ValueError("NOTION_TICKET_DATABASE_ID not set in .env")
    ...
    children = [
        {"object":"block","type":"heading_2",
         "heading_2":{"rich_text":[{"type":"text","text":{"content":"Full Question"}}]}},
        {"object":"block","type":"paragraph",
         "paragraph":{"rich_text":[{"type":"text","text":{"content":question}}]}},
        {"object":"block","type":"divider","divider":{}},
        {"object":"block","type":"heading_2",
         "heading_2":{"rich_text":[{"type":"text","text":{"content":"Conversation Summary"}}]}},
        {"object":"block","type":"paragraph",
         "paragraph":{"rich_text":[{"type":"text","text":{"content":conversation_summary[:2000]}}]}},
        {"object":"block","type":"divider","divider":{}},
        {"object":"block","type":"heading_2",
         "heading_2":{"rich_text":[{"type":"text","text":{"content":"Sources Attempted"}}]}},
    ]
    for src in sources_tried:
        children.append({
            "object":"block","type":"bulleted_list_item",
            "bulleted_list_item":{"rich_text":[{"type":"text","text":{"content":src}}]}
        })
    children.append({
        "object":"block","type":"callout",
        "callout":{
            "rich_text":[{"type":"text","text":{
                "content":f"Evidence score: {evidence_score:.3f} (threshold 0.45) — insufficient"
            }}],
...
def update_notion_ticket_status(notion_page_id: str, status: str):
    r = requests.patch(
        f"{NOTION_API}/pages/{notion_page_id}",
        headers=_headers(),
        json={"properties":{"Status":{"select":{"name":status.replace("_"," ").title()}}}},
        timeout=15,
    )
    if r.status_code != 200:
