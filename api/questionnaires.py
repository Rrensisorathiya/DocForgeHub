...
def _assert_department(department: str) -> None:
    if not _SCHEMA_AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail=(
                "Schema service unavailable — schemas/document_schema.py failed to load "
                f"or Schema/*.json files are missing. Error: {_schema_import_err_msg}. "
                "Fix: (1) replace schemas/document_schema.py with the updated version, "
                "(2) place content.json, metadata.json, Question_Answer.json in Schema/ folder, "
                "(3) restart uvicorn."
            ),
        )
