...
#         document_type,
#         f"Write a complete professional {document_type} for {company_name} in {department}."
#     )
...
# CRITICAL: Each section above requires MINIMUM 250 words.
# Write detailed procedures, real examples, tables, and checklists.
# Do NOT write brief summaries — write COMPLETE professional content.
...
# WORD COUNT REQUIREMENT: {target_words:,} words MINIMUM
# SECTION REQUIREMENT: Write EACH section with:
# - Minimum 250 words of detailed content
# - At least 1 table OR numbered list
# - Real examples with company name "{company_name}"
# - Specific procedures, policies, or guidelines
# - NO summaries, NO placeholders

# COMPLETION CHECK — Before finishing, verify:
# ✓ All sections from TOC are written
# ✓ Each section has 250+ words
# ✓ Version History table is last
...
#     # NOTE: strong ending already in base_prompt
...
#     elif max_page <= 10:
#         # 4-10 page doc: increased tokens for complete generation
#         max_tokens = max(10000, min(estimated_tokens + 4000, 12000)) if is_regeneration else max(9000, min(estimated_tokens + 3000, 11000))
#         temperature = 0.20 if is_regeneration else 0.50
...
#         max_tokens = 16000  # Maximum tokens for all long docs
...
#             "You are a senior enterprise documentation specialist. "
#             "Write COMPLETE, DETAILED, PROFESSIONAL documents. "
#             "EVERY section must be minimum 250 words with tables, examples, and procedures. "
#             "NEVER write short summaries — always write FULL detailed content. "
#             "NEVER stop until ALL sections are complete with Version History at the end. "
#         )
...
#         finish     = response.choices[0].finish_reason
#         logger.info(f"✅ Part 1: {word_count} words | finish_reason: {finish}")

#         # Always generate continuation for medium/large docs
#         part1 = clean_generated_content(response.choices[0].message.content)
#         word_count = len(part1.split())
#         finish = response.choices[0].finish_reason
#         logger.info(f"Part 1: {word_count} words | finish: {finish}")

#         # For docs needing more content — always continue
#         if max_page > 3 and word_count < (target_words * 0.8):
#             logger.info(f"Only {word_count}/{target_words} words — generating continuation...")
#             try:
...
#                         {"role": "user", "content": prompt},
#                         {"role": "assistant", "content": part1},
#                         {"role": "user", "content": f"The document is incomplete. Continue writing ALL remaining sections. Current word count is only {word_count} but we need {target_words}. Start from where you left off and complete ALL remaining sections:"},
#                     ],
#                     temperature=temperature,
#                     max_tokens=max_tokens,
#                 )
#                 part2 = clean_generated_content(cont_response.choices[0].message.content)
#                 full = part1 + "\n\n" + part2
#                 logger.info(f"Total: {len(full.split())} words after continuation")
#                 return full
#             except Exception as e:
