...
# DEFAULT_DATASET = [
#     {"question": "What are the confidentiality obligations in the NDA?",
#      "ground_truth": "The receiving party must protect confidential information with at least reasonable care and not disclose it to third parties."},
#     {"question": "What is the SLA uptime guarantee?",
#      "ground_truth": "99.9% monthly availability is guaranteed with downtime credits for violations."},
#     {"question": "What does the HR leave policy say about annual leave?",
#      "ground_truth": "Employees are entitled to paid annual leave as defined by company policy and local law."},
#     {"question": "What triggers a data breach notification obligation?",
#      "ground_truth": "Any unauthorized access to personal data must be reported to authorities within 72 hours."},
#     {"question": "What are the payment terms in the vendor contract?",
#      "ground_truth": "Payment is due within 30 days of invoice receipt unless otherwise agreed."},
# ]


# # ── heuristic RAGAS-style scorers ────────────────────────────────────────

# def _faithfulness(answer: str, chunks: list) -> float:
#     """Check what fraction of answer content appears in retrieved chunks."""
#     if not chunks or not answer:
#         return 0.0
#     chunk_text = " ".join(c.get("text", "").lower() for c in chunks)
#     words      = [w for w in answer.lower().split() if len(w) > 5]
#     if not words:
#         return 0.5
#     hits = sum(1 for w in words if w in chunk_text)
#     return min(hits / len(words) * 1.5, 1.0)
...
#     return min(overlap / len(q_words) * 2.5, 1.0)
...
#     if not gt_words:
#         return 0.5
#     hits = sum(1 for w in gt_words if w in chunk_text)
#     return min(hits / len(gt_words) * 1.8, 1.0)


# def _context_precision(chunks: list, threshold: float = 0.45) -> float:
...
#     For each question: retrieve → generate → score 4 metrics.
#     """
#     from rag.tools import search_docs, refine_query
...
#             "ground_truth":      ground_truth,
#             "refined_query":     refined,
#             "citations":         [c["doc_title"] for c in chunks[:3]],
#             "chunks_used":       len(chunks),
#             "faithfulness":      f,
#             "answer_relevancy":  r,
#             "context_recall":    rc,
#             "context_precision": p,
#         })
#         logger.info(f"Eval Q: '{question[:50]}' → F={f} R={r} RC={rc} P={p}")
...
# def load_all_results() -> list:
#     """Load all evaluation result files for history view."""
#     files   = sorted(RESULTS_DIR.glob("eval_*.json"), reverse=True)
#     results = []
#     for fp in files[:20]:   # cap at 20 most recent
#         try:
#             with open(fp) as f:
#                 data = json.load(f)
#                 data["filename"] = fp.name
